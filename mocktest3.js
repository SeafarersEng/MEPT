...
async function startExam() {
    ...
    const setIndex = Math.floor(Math.random() * 3);
    window.grammarQuestions = grammarPools[setIndex];
    window.readingPassages = readingPools[setIndex];
    window.listeningQuestions = [
        ...listeningPools[setIndex].task2.questions,
        ...listeningPools[setIndex].task3.questions
    ];
    window.writingTasks = writingPools[setIndex];
    window.speakingQuestions = speakingPools[setIndex];
    // -----------------------------
...
function loadFixedExam() {
    // ---- window ကနေ variable တွေကို ယူပါ ----
    const grammarQuestions = window.grammarQuestions;
    const readingPassages = window.readingPassages;
    const listeningQuestions = window.listeningQuestions;
    const writingTasks = window.writingTasks;
    const speakingQuestions = window.speakingQuestions;
    ...
    // ---- Speaking (ဒီနေရာမှာ speakingQuestions ကို window ကနေ ယူထားပြီးသား) ----
...
function submitExam() {
    ...
    // ---- window ကနေ variable တွေကို ယူပါ (မရှိရင် alert ပြပါ) ----
    const grammarQuestions = window.grammarQuestions;
    const readingPassages = window.readingPassages;
    const listeningQuestions = window.listeningQuestions;
    const writingTasks = window.writingTasks;
    ...
    // ---- variable တွေ ရှိမရှိ စစ်ဆေးပါ ----
    if (!grammarQuestions || !readingPassages || !listeningQuestions || !writingTasks || !speakingQuestions) {
        alert('❌ Error: Exam data not loaded. Please restart the test.');
        console.error('Missing data:', { grammarQuestions, readingPassages, listeningQuestions, writingTasks, speakingQuestions });
        return;
    }

    // ---- Speaking ရဲ့ part တွေ ရှိမရှိ စစ်ပါ ----
    if (!speakingQuestions.part1 || !speakingQuestions.part2 || !speakingQuestions.part3) {
        alert('❌ Error: Speaking sections missing.');
        console.error('Speaking data missing parts:', speakingQuestions);
        return;
    }
    ...
    // ---- Speaking (error ကာကွယ်မှုထည့်ထား) ----
    ...
    const addSpeakingScore = (arr, prefix, maxPerQ) => {
        ...
        arr.forEach((q, i) => {
            ...
            let cc = 0;
            if (q.keywords) {
                q.keywords.forEach(k => { if (text.toLowerCase().includes(k)) cc++; });
            }
    ...
    let sScore = 0;
    if (rawMax > 0) {
        sScore = Math.round((rawSpeaking / rawMax) * 20);
    } else {
        sScore = 0;
        alert('⚠️ Speaking section has no questions to score.');
    }
    total += sScore;
    max += 20;
    ...
    document.getElementById('examResult').innerHTML = `
        <div class="result-card" id="resultCard">
            ...
            <div class="result-details">
                ...
                <div class="result-item"><span class="section-name">🎧 Listening</span><span class="section-score">${lScore}/${listeningQuestions.length}</span><div class="section-bar"><div class="section-bar-fill" style="width:${Math.round(lScore/listeningQuestions.length*100)}%"></div></div></div>
        ...
        <div style="background: linear-gradient(135deg, #1a3a5c, #0f4c75); padding: 8px; border-radius: 20px; box-shadow: 0 20px 50px rgba(15, 76, 117, 0.3);">
            <div style="background: linear-gradient(180deg, #ffffff 0%, #f8f9fa 100%); border-radius: 14px; padding: 30px 25px; position: relative; overflow: hidden; border: 2px solid #f0b429; margin: 4px;">
                ...
                <div style="text-align: center;">
                    ...
                    <p style="font-size: 1.2rem; font-weight: 700; color: #f0b429; margin-bottom: 12px;">Mock Test</p>
                    ...
                    <div style="display: flex; gap: 20px; margin: 20px 0; align-items: center;">
                        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 10px; flex: 1;">
                            ...
                            <div style="background: #f8f9fa; padding: 10px; border-radius: 8px; text-align: center; border: 1px solid #e2e8f0;">
                                ...
                                <span style="font-size: 1.3rem; font-weight: 700; color: #f0b429;">${grade.text}</span>
                            </div>
                            <div style="background: #f8f9fa; padding: 10px; border-radius: 8px; text-align: center; border: 1px solid #e2e8f0;">
